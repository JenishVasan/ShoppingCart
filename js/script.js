...
const ProductCardRender = () => {
  ...
  products.forEach((element) => {
    design += `<div class="CardWrap col-3 px-2 ">
                    <div class="card ">
                        <div class="cardThumbnail position-relative  rounded-4">
                            <img class="img-fluid" src="${element.thumbnail}" alt="">

                            <div class="product-buttons position-absolute  ">
                                <div class="rounded-circle  add-to-cart-btn bg-dark  mb-3 " onclick="return cartHandler(${element.id})"  ><i class="bi bi-bag-plus"></i></div>
                                <div class="rounded-circle  product-detail-button bg-dark  mb-3" onclick="return Openmodal(${element.id})" ><i class="bi bi-eye"></i></div>
                                
                            </div>
                        </div>
                        <div class="card-Content p-3  "> 
                          <div> 
                            <p class="fw-semibold fs-5">${element.title}</p>
                            <p class="product-description">${element.description}</p>
                          </div>
                            <p class="fw-bold fs-4">Rs.${element.price}</p>
...
let viewButton = document.querySelector(".product-detail-button");
...
const cartHandler = (id) => {
  ...
  if (ProductInCart) {
    ...
    let prd = productsData.find((e) => e.id == id);
    let prdstng = {
      id: prd.id,
...
const Openmodal = (id) => {
  let product = productsData.find((e) => e.id == id);
  const imageUrl = product.thumbnail;

  modal.innerHTML = `
    <div class="modal-content-wrapper p-4 bg-white rounded-3 ">
        <div class="modal-body p-0">
            <div class="row g-4">
                <div class="col-md-4 border border-dark d-flex justify-content-center align-items-center modal-thumbnail rounded-4 shadow">
                  <img src="${imageUrl}" class="img-fluid rounded-3" alt="${product.title}">
                </div>
                <div class="col-md-7 d-flex flex-column p-5">
                    <h4 class="modal-title fw-bold fs-2">${product.title}</h4>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button class="btn btn-outline-secondary" onclick="modelHandler()">Close</button>
                    </div>
                </div>
            </div>
        </div>
...
const modelHandler = () => {
  ...
  document.querySelector(".productCard").classList.toggle("d-none");
